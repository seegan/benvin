	...
	if(isset($params['site_address']) ) {
		foreach ($params['site_address'] as $s_value) {
			...
			if($s_value['site_id'] == 0) {
				$wpdb->insert($site_table, array('customer_id' => $customer_id, 'site_name' => $s_value['site_name'], 'site_address' => $s_value['site_address'], 'phone_number' => $s_value['site_phone'], 'extra_contact' => $s_value['extra_contact'], 'discount' => $s_value['discount'], 'gst_number' => $s_value['gst_number'], 'gst_for' => $s_value['gst_for'], 'vat_number'=> $s_value['vat_number'] , 'updated_by' => $loggdin_user ));
				$site_id = $wpdb->insert_id;
				create_admin_history(array('updated_by' => $loggdin_user, 'update_in' => $site_id, 'detail' => 'site_create' ));

			} else {
				$wpdb->update($site_table, array( 'site_address' => $s_value['site_address'], 'site_name' => $s_value['site_name'], 'phone_number' => $s_value['site_phone'],'extra_contact' => $s_value['extra_contact'], 'discount' => $s_value['discount'], 'gst_number' => $s_value['gst_number'] , 'gst_for' => $s_value['gst_for'], 'vat_number'=> $s_value['vat_number'], 'active' => 1), array('id' => $s_value['site_id']));
